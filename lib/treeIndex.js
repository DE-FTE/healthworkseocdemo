...
export function getNodeContents(nodes, nodeIds) {
  // Build a lookup map so we can return nodes in the CALLER'S requested order,
  // not page order. This matters for per-doc context budget truncation:
  // route.js puts title-matched (highest-relevance) nodes first in nodeIds,
  // so preserving that order ensures the most relevant content survives the
  // char-budget slice even when a document is long.
  const nodeMap = new Map(
    nodes.filter(n => n.nodeId && n.text).map(n => [String(n.nodeId), n])
  );
  return nodeIds
